...
//start hough    
	...
	break;
      if(j = 0)
      {
	if(cvRound(circles[j][0])=='\0')
	  break;
	max = cvRound(circles[j][0])-cvRound(circles[j][1]);
      }
      Point center(cvRound(circles[j][0]), cvRound(circles[j][1]));
      radius = cvRound(circles[j][2]);
      if((cvRound(circles[j][0])-cvRound(circles[j][1]))>=max )
      {
	tempi = j;
	max = cvRound(circles[j][0])-cvRound(circles[j][1]);
	c1 = cvRound(circles[j][0]);
	c2 = cvRound(circles[j][1]);
	tempr = radius;
      }
    }
    //drawing aim
